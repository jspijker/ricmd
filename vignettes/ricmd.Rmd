...
#> Warning in dir.create(testDatadir): '/tmp/Rtmpv8iwoB/data' already exists
...
With the ri\_metaGet you can get all the meta data from an object
using the internal representation in which meta data is stored in this
package.
...
The ri\_metaGet returns a nested list with AVU tripples and keys. The
list element called 'avu' contains a list with the  AVU triple, containing
named elements for attribute, value and units. Units are by default NULL,
and the list thus contains NULL values. The list element called 'keys'
is and index with the names of the attributes and the elements of the
'avu' list in which these attributes occur.

In a future version of this package a more human readable format for
meta data is anticipated.
