...
```{r eval=FALSE}
ri_removeCollection("/tempZone/home/devel/tests")
...
no t completely true). One can store a local
...
collection set by `ri\_setCollection`. A equivalent approach can be
...
```{r eval=TRUE}
ri_setCollection("/tempZone/home/devel/tests")
...
In iRODS meta data, as AVU tripple, can be associated with users,
collections and data objects. This package only supports meta data of
data object. Other meta data is not supported (yet).
...
While it is possible in iRODS to multiple attributes with the same
name, this package does not support that. If an attribute allready
exists, adding that attribute again results in an error. It is
possible to overwrite using the overwrite argument:


```{r}
ri_metaAdd(object="datafile.rds",attribute="attr1",value="val2",
        overwrite=TRUE)
```

With the ri\_getGet and ri\_metaGetAttr you can get respectively all the meta data
AVU tripples or only a single AVU tripple for a specific attribute.

```{r}
# first add some more meta data
ri_metaAdd(object="datafile.rds",attribute="attr2",value="val2")
ri_metaAdd(object="datafile.rds",attribute="attr3",value="val3",unit="unit1")
...
The ri\_metaGet returns a list were the first levels are the names of
the attributes and the second level are the value and unit of each
level. The third meta data attribute thus can be accessed as follows:
...
```
To get a specific meta data attribute:

```{r}
ri_metaGetAtt(object="datafile.rds",attribute="attr3")

```

You can also delete meta triples from an object. To delete a single
attribute:

```{r}

ri_metaRemoveAtt("datafile.rds","attr3")
meta <- ri_metaGet("datafile.rds")
print(str(meta))
...
rm(session)
unlink(testDatadir,recursive=TRUE)
