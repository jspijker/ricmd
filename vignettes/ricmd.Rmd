...
``` r
my_collection <- "/tempZone/home/devel"
ri_setCollection(my_collection)
#> Error in ri_setCollection(my_collection): ri_setcollection: session does not exists
```
...
``` r
ri_getCollection()
#> Error in ri_getCollection(): collection not set
```
...
#> Error in ri_setCollection(test_collection): ri_setcollection: session does not exists
```
...
#> '/tmp/RtmpVeUX5Q/data/datafile.rds', probable reason 'No such file or
#> directory'
