...
#> loading ricmd python3 environment
#> use 'reticulate::py_config()' to see information about your python environment
#> Error in `ri_test_get_catalog_provider_ip()`:
#> ! ERROR, no docker irods catalog provider found
#> Error:
#> ! object 'irods_ip' not found
#> Error:
#> ! object 'env' not found
#> Error:
#> ! No valid iRODS session
#> Error:
...
#> Error:
#> ! object 'irods_env_file' not found
```
...
#> Error:
#> ! object 'irods_zone' not found
my_collection
#> Error:
#> ! object 'my_collection' not found
...
#> Error:
#> ! object 'test_collection' not found
```
...
#> Error in get("collection", envir = .ricmdEnv) : 
#>   object 'collection' not found
...
#> '/tmp/RtmpFmpRhm/data/datafile.rds', probable reason 'No such file or
#> directory'
#> Error in `gzfile()`:
...
collections and data objects. This package supports meta data of data
objects and collections. Meta data of users etc. is not supported.
...
The same workflow can be used for collections with the
ri\_metaAddColl, ri\_metaGetColl, ri\_metaExistsColl and
ri\_metaRemoveColl functions.

To add meta data to a collection, use ri\_metaAddColl. The collection
argument is mandatory and must be the full iRODS path.


``` r
ri_metaAddColl(collection = test_collection, attribute = "coll_attr1", value = "coll_val1")
#> Error in `ri_metaAddColl()`:
#> ! could not find function "ri_metaAddColl"
ri_metaAddColl(collection = test_collection, attribute = "coll_attr2", value = "coll_val2", units = "unit2")
#> Error in `ri_metaAddColl()`:
#> ! could not find function "ri_metaAddColl"
```

Use ri\_metaGetColl to retrieve all AVU tripples from the collection
as a data.frame with attribute, value and units fields. If
return\_list = TRUE, the internal AVU list is added as attribute
"av" to the returned data.frame.


``` r
meta_coll <- ri_metaGetColl(test_collection)
#> Error in `ri_metaGetColl()`:
#> ! could not find function "ri_metaGetColl"
print(meta_coll)
#> Error:
#> ! object 'meta_coll' not found
```

If needed, you can test if a specific AVU tripple exists in the
collection with ri\_metaExistsColl.


``` r
ri_metaExistsColl(collection = test_collection, attribute = "coll_attr1", value = "coll_val1")
#> Error in `ri_metaExistsColl()`:
#> ! could not find function "ri_metaExistsColl"
ri_metaExistsColl(collection = test_collection, attribute = "coll_attr2", value = "coll_val2", units = "unit2")
#> Error in `ri_metaExistsColl()`:
#> ! could not find function "ri_metaExistsColl"
```

To remove a collection AVU tripple, use ri\_metaRemoveColl.


``` r
ri_metaRemoveColl(collection = test_collection, attribute = "coll_attr2", value = "coll_val2", units = "unit2")
#> Error in `ri_metaRemoveColl()`:
#> ! could not find function "ri_metaRemoveColl"
meta_coll <- ri_metaGetColl(test_collection)
#> Error in `ri_metaGetColl()`:
#> ! could not find function "ri_metaGetColl"
print(meta_coll)
